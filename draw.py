...
def crop_around_point(img, row, col, padding=100):
    h, w = img.shape[:2]

    top = max(row - padding, 0)
    bottom = min(row + padding, h)
    left = max(col - padding, 0)
    right = min(col + padding, w)
...
def get_top_down_map(sim, map_resolution=1024, meters_per_pixel=0.05, base_height=None):
    if base_height == None:
...
def get_waypoints_from_radial_map(radial_map, num_angles, num_distances, max_dist=3):
    waypoints = []
    angle_resolution = 2 * np.pi / num_angles
    dist_resolution = max_dist / num_distances
    for angle_idx in range(num_angles):
        for dist_idx in range(num_distances):
            if radial_map[angle_idx, dist_idx] > 0:  
                r = (dist_idx + 0.5) * dist_resolution
                theta = angle_idx * angle_resolution
                waypoints.append((r, theta))
    return waypoints

def get_obstacles_from_radial_map(radial_map, num_angles, num_distances, max_dist=3):
    obstacles = []
    angle_resolution = 2 * np.pi / num_angles
    dist_resolution = max_dist / num_distances
    for angle_idx in range(num_angles):
        for dist_idx in range(num_distances):
            if radial_map[angle_idx, dist_idx] > 0:  
                r = (dist_idx + 0.5) * dist_resolution
                theta = angle_idx * angle_resolution
                obstacles.append((r, theta))
                break
            if dist_idx == num_distances - 1:
                obstacles.append((3, angle_idx * angle_resolution))
    return obstacles

def get_distidx_theta(radial_map, num_angles, num_distances):

    angle_resolution = 2 * np.pi / num_angles
    angle_indices, dist_indices = np.nonzero(radial_map)
    thetas = angle_indices * angle_resolution
    waypoints = list(zip(dist_indices, thetas))
...
def draw_waypoint_prediction_new(
    img: np.ndarray,
    waypoint: Union[Tuple[float], List[float]],
    meters_per_px: float,
    sim
) -> None:
    w_x, w_y = habitat_maps.to_grid(waypoint[1], waypoint[0], img.shape[0:2], sim)
    if w_x < img.shape[0] and w_x > 0 and w_y < img.shape[1] and w_y > 0:
        drawpoint(img, (w_x, w_y), MAP_WAYPOINT_PREDICTION, meters_per_px, pad=0.05)
...
def draw_oracle_waypoint_new(
    ...
    w_x, w_y = habitat_maps.to_grid(waypoint[1], waypoint[0], img.shape[0:2], sim)
    if w_x < img.shape[0] and w_x > 0 and w_y < img.shape[1] and w_y > 0:
        drawpoint(img, (w_x, w_y), MAP_ORACLE_WAYPOINT, meters_per_px, pad=0.05)
...
def draw_source_new(
    ...
    w_x, w_y = habitat_maps.to_grid(waypoint[1], waypoint[0], img.shape[0:2], sim)
    if w_x < img.shape[0] and w_x > 0 and w_y < img.shape[1] and w_y > 0:
        drawpoint(img, (w_x, w_y), MAP_SOURCE_POINT_INDICATOR, meters_per_px, pad=0.05)

def draw_obstacle_new(
    img: np.ndarray,
    waypoint: Union[Tuple[float], List[float]],
    meters_per_px: float,
    sim
    ...
    w_x, w_y = habitat_maps.to_grid(waypoint[1], waypoint[0], img.shape[0:2], sim)
    if w_x < img.shape[0] and w_x > 0 and w_y < img.shape[1] and w_y > 0:
        drawpoint(img, (w_x, w_y), MAP_TARGET_POINT_INDICATOR, meters_per_px, pad=0.02)
