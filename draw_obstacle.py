...
for _, data in enumerate(evalloader):
    ...
    vis_probs, vis_logits, obs_probs, obs_logits = TRM_predict('eval', args, predictor, rgb_feats, depth_feats)
    ...
    for i in range(len(scan_ids)):
        ...
        cur_obs_logits = obs_logits[i]
        cur_obs_preds_index = torch.argmax(cur_obs_logits, dim=-1)
        cur_obs_preds = torch.zeros_like(cur_obstacle)
        for j in range(cur_obs_preds.size(0)):
            cur_obs_preds[j, cur_obs_preds_index[j]:] = 1
